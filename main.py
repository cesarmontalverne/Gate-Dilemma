...
def startscreen(camera):
    ...
    instructions = ["Choose a logical gate that matches the inputs and the output."
...
def play(camera):
    ...
    camera.draw(gamebox.from_text(133, 550, 'NOR(8)', 40, 'white'))
    camera.draw(gamebox.from_text(400, 550, 'XOR(9)', 40, 'black'))
    camera.draw(gamebox.from_text(667, 550, 'XNOR(0)', 40, 'white'))
...
async def tick():
    '''
    Runs the game, adapted for async.
    '''
    global game_on, timer, vari, camera
